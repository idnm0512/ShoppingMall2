...
<body>
	...
	<main>
		...
		<button class="commMgtWrite_btn" style="margin-left: 10%;">글쓰기</button>
	...
	<script>
		...
		$('.move_detail').on('click', function(e){
			...
			pageInfo_form.attr('action', '/commMgt/detail');
		...
		$('.pageInfo_ul a').on('click', function(e){
		    ...
		    // f.attr("action", "/admin/commMgtList");
		    pageInfo_form.submit();
		});
		
		// 글쓰기 버튼 클릭 -> 커뮤 작성 페이지 진입
		$('.commMgtWrite_btn').on('click', function(e){
			e.preventDefault();
			
			pageInfo_form.attr('action', '/commMgt/write');
			pageInfo_form.submit();
		});
	</script>
</body>
